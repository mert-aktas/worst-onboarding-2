...
const Game = {
  ...
  showPayoff() {
    ...
    screen.innerHTML = `
      <a class="payoff-skip" id="payoff-skip" data-valid-click>Pular</a>
      <div class="payoff-stage">
        <div class="payoff-tooltip" id="payoff-tooltip">Crie seu primeiro projeto. Dois passos, só isso.</div>
        <div class="payoff-field" id="payoff-field">
          <input class="payoff-input" id="payoff-input" placeholder="Nome do projeto" readonly>
          <button class="payoff-create" id="payoff-create">Criar</button>
        </div>
        <div class="payoff-success hidden" id="payoff-success">
          ...
          <p class="payoff-checklabel" id="payoff-checklabel">Configuração completa, 0 de 2</p>
        </div>
        <div class="payoff-headline hidden" id="payoff-headline">
          <h2>É isso. É assim que um onboarding deveria ser.</h2>
          <button class="payoff-continue" id="payoff-continue" data-valid-click>Ver meus resultados →</button>
        </div>
      </div>
    `;
    screen.classList.remove('hidden');
    ...
    createBtn.addEventListener('click', () => {
      ...
      beats.push(setTimeout(() => { document.getElementById('payoff-c1').textContent = "✓ Projeto criado"; document.getElementById('payoff-checklabel').textContent = "Configuração completa, 1 de 2"; }, 500));
      beats.push(setTimeout(() => { document.getElementById('payoff-c2').textContent = "✓ Você está pronto"; document.getElementById('payoff-checklabel').textContent = "Configuração completa, 2 de 2"; }, 1200));
    ...
    beats.push(setTimeout(() => {
      const text = "Meu primeiro projeto";
      let i = 0;
      const type = () => {
        if (finished) return;
  ...
  showResults() {
    ...
    if (false) {  // PT-BR uses the Yasmin capture card (capture.js), not the meetings embed
  ...
  saveBestScore(time, clicks, patience) {
    const prev = JSON.parse(localStorage.getItem('woe2-best-ptbr') || 'null');
    if (!prev || time < prev.time) {
      localStorage.setItem('woe2-best-ptbr', JSON.stringify({ time, clicks, patience }));
